import React from 'react'
import "./Home.css";
import Product from './Product';

function Home() {
    return (
        <div className = "home">
            
          <img className ="home_image" 
          src = "https://images-eu.ssl-images-amazon.com/images/G/02/digital/video/merch2016/Hero/Covid19/Generic/GWBleedingHero_ENG_COVIDUPDATE__XSite_1500x600_PV_en-GB._CB428684220_.jpg"
          alt =""
          />

          {/* Product ID, title, price, reating,image */}

          <div className = "home_row">

          <Product 
            id ="1"
            title="Orignal SS Cricket Bat Along With Signature Of MS Dhoni  "
            price= {70.96}
            rating = {5}
            image="https://encrypted-tbn0.gstatic.com/images?q=tbn%3AANd9GcShQsJntCmXac-ESWnzc9jS2UzX8DfznkTR3Q&usqp=CAU"
         />

          <Product 
            id ="2"
            title="Brand New MI Watches With Much More Specification"
            price= {90.96}
            rating = {5}
            image="https://encrypted-tbn0.gstatic.com/images?q=tbn%3AANd9GcRQkFEXRZUUxpWLlblboNBRyoy89n0QbbMQ4w&usqp=CAU"

         />
        </div>

        <div className = "home_row">
        <Product 
            id ="3"
            title="Remote For Amazon MI Television"
            price= {110.96}
            rating = {5}
            image="data:image/jpeg;base64,"

         />

          <Product 
            id ="4"
            title="Amazing Sound Quality Bluetoth Speakers"
            price= {300.96}
            rating = {5}
            image="https://encrypted-tbn0.gstatic.com/images?q=tbn%3AANd9GcQl_QAjkSNNEZ5hBw8wxoKm8MOMjtv8I4n29A&usqp=CAU"
          />

          <Product 
            id ="5"
            title="1 Ton Split AC With Best Ratings "
            price= {965.96}
            rating = {5}
            image="https://encrypted-tbn0.gstatic.com/images?q=tbn%3AANd9GcSP7YS0ns1wkid95rdN2q9u3fNFsF-qhDTaRw&usqp=CAU"
          />

        </div>


        <div className = "home_row">
        <Product 
            id ="5"
            title="Amazing Sony Bravia 55 Inch TV Make Your Home As Theater "
            price= {1567.96}
            rating = {5}
            image="https://encrypted-tbn0.gstatic.com/images?q=tbn%3AANd9GcSlB-Sl3LUS2iPYAYMTNb--Up8FK76xXzb9xw&usqp=CAU"
          />
        </div>

         
        
        </div>
    )
}

export default Home
